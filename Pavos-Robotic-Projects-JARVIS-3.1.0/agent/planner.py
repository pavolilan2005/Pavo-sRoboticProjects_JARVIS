...
PLANNER_PROMPT = """You are the planning module of PAVO'S ROBOTIC PROJECTS // JARVIS, a modular personal AI assistant.
...
run_routine
  routine: string (required) — configured routine name such as modo_stream
...
media_control
  action: "status" | "play_track" | "play_playlist" | "play" | "pause" | "next" | "previous" | "volume" | "save"
  query: string (for play_track)
  playlist: string (for play_playlist)
  volume: integer 0-100 (for volume)

obs_control
  action: "status" | "open" | "set_scene" | "start_recording" | "stop_recording" | "start_stream" | "stop_stream"
  scene: string (for set_scene)
  confirmed: boolean (required true for start/stop stream after explicit user confirmation)

pc_status
  no parameters

email_center
  action: "status" | "unread" | "search" | "read" | "mark_read"
  query, limit, message_id: optional

notification_center
  action: "list" | "read" | "windows" | "emit"
  limit, title, message, priority: optional

home_automation
  device: string (required)
  action: "on" | "off" | "toggle" | "status" | "set" (required)
  value: optional
EXAMPLES:
