...
class PavoHudCanvas(QWidget):
    ...
    def set_audio_level(self, level: float, rms: float = 0.0):
        self.mic_level = max(0.0, min(100.0, float(level)))
        self.mic_rms = max(0.0, float(rms))

    def set_standby_remaining(self, seconds: float):
        self.standby_remaining = max(0.0, float(seconds))

    def _state_detail(self) -> str:
        if self.muted:
            return "AUDIO DE ENTRADA BLOQUEADO"
        if self.state == "SLEEPING":
            return "STANDBY · DI 'JARVIS' PARA ACTIVAR"
        if self.state == "LISTENING":
            if self.standby_remaining > 0:
                return f"VENTANA ACTIVA · STANDBY EN {int(self.standby_remaining + 0.999)}s"
            return "ESCUCHANDO · DI UNA ORDEN"
        if self.state in ("THINKING", "PROCESSING"):
            return "PROCESANDO SOLICITUD"
        if self.state == "SPEAKING":
            return "TRANSMISIÓN DE VOZ ACTIVA"
        if self.state == "OFFLINE":
            return "SIN CONEXIÓN A GEMINI"
        if self.state == "CONNECTING":
            return "ESTABLECIENDO ENLACE"
        return "VOICE LINK · ESP32 SERIAL · DOMÓTICA"
    ...
    def _step(self):
        ...
        if self.muted:
            ...
            self._target_amp = max(0.38, min(0.96, 0.34 + self.mic_level / 120.0))
        ...
        for i, v in enumerate(self._bars):
            if self.speaking:
                ...
                live = min(1.0, self.mic_level / 100.0) if self.state == "LISTENING" else 0.0
                target = 0.16 + live * random.uniform(0.25, 0.95) + 0.16 * math.sin(self._tick * 0.045 + i * 0.42)
                speed = 0.20 if live > 0.03 else 0.12
    ...
    def paintEvent(self, _):
        ...
        p.drawText(QRectF(card.x(), card.y() + 30, card.width(), 14), Qt.AlignmentFlag.AlignCenter, self._state_detail())
...
class MainWindow(QMainWindow):
    ...
    _audio_level_sig = pyqtSignal(float, float)
    _standby_sig = pyqtSignal(float)

    def __init__(self, face_path: str):
        super().__init__()
        self.setWindowTitle("PAVO'S ROBOTIC PROJECTS // JARVIS")
        ...
        self._standby_sig.connect(self._apply_standby)
    ...
    def _open_control_center(self):
        if self.platform is None:
            ...
            return
        try:
            from prp.ui.control_center import ControlCenterDialog
    ...
    def _apply_audio_level(self, level: float, rms: float):
        self.hud.set_audio_level(level, rms)

    def _apply_standby(self, seconds: float):
        self.hud.set_standby_remaining(seconds)
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("SLEEPING")
        self._log.append_log(f"SYS: Initialised. OS={os_name.upper()}. Pavo's Robotic Projects / JARVIS online. Say 'Jarvis' to wake.")
...
class JarvisUI:
    ...
    def set_audio_level(self, level: float, rms: float = 0.0):
        self._win._audio_level_sig.emit(float(level), float(rms))

    def set_standby_countdown(self, seconds: float):
        self._win._standby_sig.emit(float(seconds))
